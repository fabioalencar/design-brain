...
Opening a drawer must not re-fetch or re-render the list underneath in a way that loses scroll, expanded rows, checked rows or the active filters; the row the drawer represents stays highlighted while it is open. When the drawer's edit is saved, the row updates in place and the list does not jump. This is the reason to choose a drawer over a page, so violating it removes the benefit.
